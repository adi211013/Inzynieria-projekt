...
async function maybeCompleteGoal(goalId: number): Promise<void> {
  const goal = await prisma.goals.findUnique({ where: { goal_id: goalId } });
  if (!goal || goal.status !== 'in_progress') return;

  const completedDays = await prisma.goal_logs.count({
...
export const getLogs = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const [logs, total] = await Promise.all([
      prisma.goal_logs.findMany({
        where,
        orderBy: { date: 'desc' },
        skip: (page - 1) * limit,
        take: limit,
      }),
      prisma.goal_logs.count({ where }),
    ]);
